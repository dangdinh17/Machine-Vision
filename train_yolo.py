...
def receive_arg():
    ...
    parser.add_argument('--opt_path', type=str, default='configs/train_yolo.yml', help='Path to option YAML file.')
...
def main():
    ...
    train_dataset = utils.YOLOTrainDataset(opts_dict['dataset']['train']['hr_train'],
    ...
    for epoch in range(start_epoch, num_epoch):
        ...
        for i, (hr_images, targets) in enumerate(tqdm(train_loader, desc=f'Epoch {epoch+1}/{num_epoch}', unit='batch')):
            # get data
            hr_images = hr_images.to(rank)  # (B T C H W)s
            
            pred = detection(hr_images)
            batch = {"img": hr_images, **targets}

            machine_loss, _ = detection_loss(pred, batch)
            total_loss = machine_loss.sum()
