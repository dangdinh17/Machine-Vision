...
def import_yuv(seq_path, h, w, tot_frm, yuv_type='420p', start_frm=0, only_y=True):
    """Load Y, U, and V channels separately from a 8bit yuv420p video. (限制了 yuv 的格式)
    参数:
        seq_path (str): .yuv（imgs）路径。
        h (int): 高度。
        w (int): 宽度。
        tot_frm (int): 要导入的总帧数。
        yuv_type: 420p或444p
        start_frm (int): 要导入的第一帧。默认为0。
        only_y (bool): 仅导入Y通道。
        返回:
        y_seq，u_seq，v_seq（3个通道的3个ndarray）: Y通道，U通道，V通道。
    Note:
...
# 将Y、Cb和Cr三个通道的图像数据写入一个二进制文件（vid_path）
...
class _LmdbBackend:
    """Lmdb storage backend.
        参数:
            ...
            readonly (bool, 可选): Lmdb环境参数。如果为True，禁止任何写入操作。默认值: True。
            lock (bool, 可选): Lmdb环境参数。如果为False，在发生并发访问时，不锁定数据库。默认值: False。
            readahead (bool, 可选): Lmdb环境参数。如果为False，禁用操作系统文件系统的预读机制，这可能提高数据库大于RAM时的随机读取性能。默认值: False。
        属性:
            db_paths (str): Lmdb数据库路径。
    """
